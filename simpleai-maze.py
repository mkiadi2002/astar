...
class MazeSolver(SearchProblem):  # the class MazeSolver contains the methods that we need to solve the problem
  ...
  def __init__(self, board, agent_start, agent_finish):
    ...
    for y in range(len(self.board)):
      ...
      for x in range(len(self.board[y])):
        ...
        if self.board[y][x].lower() == agent_start:
          self.initial = (x, y)
          print("")
          print("%% found o %%")
        ...
        #                    self.initial = (x, y)
        elif self.board[y][x].lower() == agent_finish:
          self.goal = (x, y)
  ...
  def actions(self, state):
    ...
    for action in COSTS.keys():  # COSTS is a dictoinary defined later down
      newx, newy = self.result(state,
                               action)  # result method is defined in the following. It gets a state and generates an action
...
def load_config(agent_name, config_path):
  from yaml import safe_load
  with open(config_path, 'r') as maze_config:
    data_loaded = safe_load(maze_config)
    print(data_loaded)
    map = data_loaded["map"]
    all_agent_config = data_loaded["agent_config"]
    for agent in all_agent_config:
      if agent["name"] == agent_name:
        agent_config = agent

  return map, agent_config["start"], agent_config["finish"], agent_config["path_indicator"]

if __name__ == "__main__":

  agent_name = os.getenv("agent_name")
  MAP1, start, finish, path_indicator = load_config(agent_name, '/etc/config/maze_config.yaml')
  # while True:
  #   time.sleep(5)
  ...
  # start = os.getenv("agent_start")
  # finish = os.getenv("agent_finish")
  # map
  problem1 = MazeSolver(MAP1, start, finish)
  #    problem1.start()
  #    problem1.join()
